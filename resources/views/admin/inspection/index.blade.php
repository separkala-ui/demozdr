...
<div class="container mx-auto p-6">
                    ...
                    @foreach($reports as $report)
                        <tr class="border-t hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm">
                                <span class="px-2 py-1 bg-yellow-100 text-yellow-800 rounded text-xs">{{ $report->status }}</span>
                            </td>
                            <td class="px-6 py-4 text-sm">
                                <a href="{{ route('admin.inspection.show', $report->id) }}" class="text-blue-600 hover:underline">{{ __('مشاهده') }}</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    @else
        <div class="bg-gray-50 rounded-lg p-8 text-center">
            <p class="text-gray-500">{{ __('بازرسی یافت نشد') }}</p>
        </div>
    @endif
</div>
@endsection
