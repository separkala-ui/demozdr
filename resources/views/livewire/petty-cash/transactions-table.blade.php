        ...
        @forelse($transactions as $transaction)
            ...
            <div class="rounded-xl border border-slate-100 p-4">
                ...
                <div class="mt-3 flex flex-wrap gap-3 text-xs font-semibold">
                    ...
                    @if($isManager)
                        <button type="button"
                                wire:click="requestApprove({{ $transaction->id }})"
                                onclick="console.log('Approve clicked for transaction {{ $transaction->id }}')"
                                class="text-green-600 hover:text-green-800">
                            {{ __('تایید') }}
                        </button>
                        <button type="button" wire:click="openRevisionModal({{ $transaction->id }})" class="text-amber-600 hover:text-amber-800">
                        ...
                        <button type="button"
                                wire:click="requestReject({{ $transaction->id }})"
                                onclick="console.log('Reject clicked for transaction {{ $transaction->id }}')"
...
@endif
    ...
    @php
        $rejectTransaction = $rejectTransactionId ? $this->ledger->transactions()->find($rejectTransactionId) : null;
    @endphp
    <div
    ...
    x-show="show"
    x-cloak
    @open-reject-modal.window="show = true; console.log('Reject modal opened via Alpine');"
    style="display: none;"
>
    ...
    @php
        $approvalTransaction = $approvalTransactionId ? $this->ledger->transactions()->find($approvalTransactionId) : null;
    @endphp
    <div
        class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60"
    wire:keydown.escape.window="closeApproveModal"
    x-data="{ show: @entangle('showApproveModal') }"
    x-show="show"
    x-cloak
    @open-approve-modal.window="show = true; console.log('Approve modal opened via Alpine');"
    style="display: none;"
>
    ...
    <!-- Revision Modal -->
    @php
        $revisionTransaction = $revisionTransactionId ? $this->ledger->transactions()->find($revisionTransactionId) : null;
    @endphp
    <div
        class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60"
        wire:keydown.escape.window="$set('showRevisionModal', false)"
        x-data="{ show: @entangle('showRevisionModal') }"
        x-show="show"
        x-cloak
        style="display: none;"
    >
        <div class="absolute inset-0" wire:click="$set('showRevisionModal', false)"></div>
        <div class="relative z-10 w-full max-w-lg rounded-xl bg-white p-6 shadow-lg">
            <div class="flex items-start justify-between gap-4 border-b border-slate-200 pb-3">
                <div>
                    <h2 class="text-lg font-semibold text-slate-800">{{ __('ارسال برای بازبینی') }}</h2>
                    <p class="mt-1 text-xs text-slate-500">
                        {{ __('لطفاً دلیل ارسال به بازبینی را برای کاربر شعبه توضیح دهید.') }}
                    </p>
                </div>
                <button type="button" wire:click="$set('showRevisionModal', false)" class="rounded-full bg-slate-100 p-2 text-slate-500 hover:bg-slate-200">
                    <iconify-icon icon="lucide:x" class="text-lg"></iconify-icon>
                </button>
            </div>
            ...
            <div class="mt-4 space-y-3 text-sm text-slate-600">
                @if($revisionTransaction)
                    <div class="rounded-lg border border-amber-200 bg-amber-50 p-3 text-[13px]">
                        <div class="flex items-center justify-between">
                            <span class="text-amber-600">{{ __('شماره مرجع') }}</span>
                            <span class="font-semibold text-slate-800">{{ $revisionTransaction->reference_number ?: '---' }}</span>
                        </div>
                        <div class="mt-2 flex items-center justify-between">
                            <span class="text-amber-600">{{ __('مبلغ') }}</span>
                            <span class="font-semibold text-amber-800">{{ number_format($revisionTransaction->amount) }} {{ __('ریال') }}</span>
                        </div>
                        <div class="mt-2">
                            <span class="text-amber-600">{{ __('شرح') }}:</span>
                            <p class="mt-1 text-slate-700">{{ $revisionTransaction->description ?: '---' }}</p>
                        </div>
                    </div>
                @endif

                <div>
                    <label class="text-xs font-semibold text-slate-700" for="revision-note">
                        {{ __('دلیل ارسال برای بازبینی') }}
                        <span class="text-red-500">*</span>
                    </label>
                    <textarea 
                        id="revision-note" 
                        rows="4" 
                        wire:model.defer="revisionNote" 
                        class="mt-1 block w-full rounded-md border-slate-300 px-3 py-2 text-sm shadow focus:border-amber-500 focus:ring-amber-500" 
                        placeholder="{{ __('مثال: لطفاً فاکتور اصلی را آپلود کنید یا مبلغ را اصلاح نمایید...') }}"
                    ></textarea>
                    @error('revisionNote')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-[11px] text-slate-500">
                        {{ __('این پیام به کاربر شعبه نمایش داده خواهد شد.') }}
                    </p>
                </div>
            </div>

            <div class="mt-6 flex flex-row-reverse justify-start gap-3">
                <button 
                    type="button" 
                    wire:click="requestRevision" 
                    class="rounded-lg bg-amber-500 px-6 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2"
                >
                    <span class="flex items-center gap-2">
                        <iconify-icon icon="lucide:send" class="text-base"></iconify-icon>
                        <span>{{ __('ارسال به بازبینی') }}</span>
                    </span>
                </button>
                <button 
                    type="button" 
                    wire:click="$set('showRevisionModal', false)" 
                    class="rounded-lg border border-slate-300 bg-white px-6 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2"
                >
                    {{ __('انصراف') }}
                </button>
            </div>
