...
class SmartInvoiceService
{
    ...
    protected function parseJsonContent(string $content): ?array
    {
        ...
        $repairedLocally = $this->basicJsonRepair($this->normaliseJsonWrapper($content));
        if ($repairedLocally !== null) {
            return $repairedLocally;
        }

        return $this->repairMalformedJson($content);
    }
    ...
    protected function reconcileFinancialSummary(array $financial, array $lineItems): array
    {
        ...
        if (! isset($financial['final_amount']) && $computedFinal !== null) {
            $financial['final_amount'] = $computedFinal;
        }

        if (isset($financial['final_amount'], $computedFinal)) {
            $difference = abs($financial['final_amount'] - $computedFinal);
            if ($difference > $tolerance) {
                $financial['difference'] = $difference;
                $financial['issues'][] = __('اختلاف جمع نهایی با اجزای محاسباتی بیش از حد مجاز است.');
            }
