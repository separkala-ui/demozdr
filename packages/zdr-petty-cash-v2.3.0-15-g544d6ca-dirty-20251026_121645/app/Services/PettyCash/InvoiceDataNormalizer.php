...
class InvoiceDataNormalizer
{
    public function normalize(array $rawData): array
    {
        ...
        $itemsResult = $this->extractItems($data);
        $metadata = $this->extractMetadata($data);
        if (! empty($itemsResult['summary_rows'])) {
            $metadata['summary_rows'] = $itemsResult['summary_rows'];
        }
    ...
    private function extractItems(array $data): array
    {
        ...
        foreach ($rawItems as $item) {
            ...
            $normalizedDescription = $description ? mb_strtolower(trim($description)) : null;
            ...
            $isSummaryCandidate = $normalizedDescription && ($quantity === null || $quantity === 0.0);
            $summaryType = null;

            if ($isSummaryCandidate) {
                if (str_contains($normalizedDescription, 'مالیات') || str_contains($normalizedDescription, 'عوارض')) {
                    $summaryType = 'tax';
                } elseif (str_contains($normalizedDescription, 'تخفیف')) {
                    $summaryType = 'discount';
                } elseif (str_contains($normalizedDescription, 'قابل پرداخت') || str_contains($normalizedDescription, 'جمع کل') || str_contains($normalizedDescription, 'مبلغ نهایی')) {
                    $summaryType = 'final_total';
                }
            }
            ...
            if ($summaryType !== null) {
                $summaryRows[] = [
                    ...
                    'amount' => $this->parseAmount($this->findValue($item, [
