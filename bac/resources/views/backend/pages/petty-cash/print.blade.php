...
@php
    ...
    $defaultNote = match ($period) {
        ...
        'archive' => 'گزارش سند تسویه شماره #' . ($cycle?->id ?? '-') . ' از ' . ($cycle?->opened_at ? verta($cycle->opened_at)->format('Y/m/d') : '---') . ' تا ' . ($cycle?->closed_at ? verta($cycle->closed_at)->format('Y/m/d H:i') : '---'),
        default => 'دوره گزارش: از ' . ($dateFrom ?? 'شروع') . ' تا ' . ($dateTo ?? 'اکنون'),
    };
    $defaultReportDate = verta()->format('Y/m/d');
    $custodian = $ledger->assignedUser ?? null;
    $custodianName = $custodian?->full_name
        ?? (trim(($custodian?->first_name ?? '') . ' ' . ($custodian?->last_name ?? '')) ?: null)
        ?? $custodian?->name
        ?? $custodian?->email
        ?? __('مسئول انتخاب نشده');
    $defaultManager = $custodianName;
    $approverName = null;
    if (!empty($isArchiveView) && $cycle?->closer) {
        $approverName = $cycle->closer->full_name ?? $cycle->closer->name ?? $cycle->closer->email;
    }
...
<div class="sheet" id="reportSheet">
    <header class="report-header">
        <div>
            ...
            <p id="reportCustodian">{{ __('مسئول تنخواه: :name', ['name' => $custodianName]) }}</p>
    ...
    @if(!empty($isArchiveView) && $cycle)
        @php
            $summary = $cycle->summary ?? [];
            $archiveBalances = $summary['balances'] ?? [];
            $archiveTransactions = $summary['transactions'] ?? [];
        @endphp
        <div class="summary-box">
            <h3>{{ __('جزئیات سند تسویه دوره') }}</h3>
            <div class="summary-grid">
                <div class="summary-item">
                    <strong>{{ __('شروع دوره') }}</strong>
                    {{ $cycle->opened_at ? verta($cycle->opened_at)->format('Y/m/d H:i') : '---' }}
                </div>
                <div class="summary-item">
                    <strong>{{ __('پایان دوره') }}</strong>
                    {{ $cycle->closed_at ? verta($cycle->closed_at)->format('Y/m/d H:i') : '---' }}
                </div>
                <div class="summary-item">
                    <strong>{{ __('مانده ابتدای دوره') }}</strong>
                    {{ number_format($archiveBalances['opening'] ?? $cycle->opening_balance ?? 0) }} {{ __('ریال') }}
                </div>
                <div class="summary-item">
                    <strong>{{ __('مانده پایان دوره') }}</strong>
                    {{ number_format($archiveBalances['closing'] ?? $cycle->closing_balance ?? 0) }} {{ __('ریال') }}
                </div>
                <div class="summary-item">
                    <strong>{{ __('تعداد تراکنش‌ها') }}</strong>
                    {{ $archiveTransactions['count'] ?? $cycle->transactions_count ?? 0 }}
                </div>
                <div class="summary-item">
                    <strong>{{ __('جمع ورودی‌ها') }}</strong>
                    {{ number_format($archiveTransactions['total_charges'] ?? $cycle->total_charges ?? 0) }} {{ __('ریال') }}
                </div>
                <div class="summary-item">
                    <strong>{{ __('جمع خروجی‌ها') }}</strong>
                    {{ number_format($archiveTransactions['total_expenses'] ?? $cycle->total_expenses ?? 0) }} {{ __('ریال') }}
                </div>
                <div class="summary-item">
                    <strong>{{ __('تعدیلات ثبت شده') }}</strong>
                    {{ number_format($archiveTransactions['total_adjustments'] ?? $cycle->total_adjustments ?? 0) }} {{ __('ریال') }}
                </div>
                <div class="summary-item">
                    <strong>{{ __('تایید کننده') }}</strong>
                    {{ $approverName ?? __('نامشخص') }}
                </div>
                <div class="summary-item">
                    <strong>{{ __('مسئول تنخواه شعبه') }}</strong>
                    {{ $custodianName }}
                </div>
            </div>
            @if($cycle->closing_note)
                <p class="mt-3 text-sm text-slate-600">{{ __('یادداشت مدیر') }}: {{ $cycle->closing_note }}</p>
            @endif
        </div>
    @endif
